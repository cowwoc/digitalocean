...
public final class Droplet
{
	 ...
	 */
	public static DropletBuilder builder(DigitalOceanScope scope, String name, DropletType type,
		DropletImage image, Vpc vpc)
	{
		return new DropletBuilder(scope, name, type, image, vpc);
	}
	 ...
	 */
	public static Droplet getById(DigitalOceanScope scope, int id)
		throws IOException, TimeoutException, InterruptedException
	{
		...
		return getByJson(scope, droplet);
	}
	 ...
	 *                                  happen due to shutdown signals.
	 */
	public static Droplet getByAddress(DigitalOceanScope scope, String address)
		throws IOException, TimeoutException, InterruptedException
	{
	 ...
	 */
	static Droplet getByJson(DigitalOceanScope scope, JsonNode json)
		throws IOException, InterruptedException, TimeoutException
	{
		...
		Vpc vpc;
		if (vpcNode != null)
			vpc = Vpc.getById(scope, vpcNode.textValue());
		else
			vpc = Vpc.getDefault(scope, zone);
		...
		return new Droplet(scope, id, name, createdAt, type, image, zone, vpc, addresses, features, tags);
	}
	 ...
	 *                                  happen due to shutdown signals.
	 */
	public static List<Droplet> getByName(DigitalOceanScope scope, String name)
		throws IOException, TimeoutException, InterruptedException
	{
		...
		return DigitalOceans.getElements(scope, uri, Map.of(), body ->
		{
			...
			for (JsonNode droplet : body.get("droplets"))
			{
				...
				if (actualName.equals(name))
					droplets.add(getByJson(scope, droplet));
			}
			return droplets;
		});
	}
	 ...
	 *                                  happen due to shutdown signals.
	 */
	public static List<Droplet> getByTags(DigitalOceanScope scope, Collection<String> tags)
		throws IOException, TimeoutException, InterruptedException
	{
		requireThat(tags, "tags").isNotEmpty();
		for (String tag : tags)
			requireThat(tag, "tag").isStripped().isNotEmpty();

		// https://docs.digitalocean.com/reference/api/api-reference/#operation/droplets_list
		String uri = REST_SERVER + "/v2/droplets";
		return DigitalOceans.getElements(scope, uri, Map.of("tag_name", tags), body ->
		{
			List<Droplet> droplets = new ArrayList<>();
			for (JsonNode droplet : body.get("droplets"))
				droplets.add(getByJson(scope, droplet));
	 ...
	 * @see Vpc#getDefault(DigitalOceanScope, Zone)
	 */
	private Droplet(DigitalOceanScope scope, int id, String name, Instant createdAt, DropletType type,
	 ...
	 */
	public boolean matchesState(DropletBuilder state)
	{
	...
	public Droplet renameTo(String newName) throws IOException, TimeoutException, InterruptedException
	{
		...
		JsonNode responseBody = DigitalOceans.getResponseBody(scope, serverResponse);
		...
		while (status.equals("in-progress"))
		{
			...
			request = DigitalOceans.createRequest(scope, uri);
